...
class TicTacToe:
    ...
    def player_move(self, player, tile_number):
        selected_position = self.tile_number_to_position(tile_number)
        row = selected_position[0]
        col = selected_position[1]
        self.board[row][col] = self.TILE_SYMBOL[player]

    def is_winning(self, player):
        players_symbol = self.TILE_SYMBOL[player]
    ...
    def play(self):
        game_over = False
        turn = self.PLAYER1  # Turn is alternating between 1 and 2 to represent player's turn
        ...
        while True:
            ...
            self.player_move(turn, choice)

            # Check winning conditions
            if self.is_winning(turn):
                self.display_board()
